...
cd "$DIR"
MAIN_REPRO="git@github.com:hudora/taskmaster.git"
...
if [ ! -e src/$PROJECT/.git ] ; then
    #git clone --recursive $MAIN_REPRO src/zamboni-src/zamboni
    git clone $MAIN_REPRO src/$PROJECT
...
if [ ! -L app.ini ] ; then
    rm app.ini
    ln -s src/$PROJECT/silver-app.ini app.ini
fi
...
if [ ! -e lib/python/$PROJECT.pth ] ; then
    echo "../../src/$PROJECT/" > lib/python/$PROJECT.pth
fi
...
if [ ! -L bin/manage.py ] ; then
    if [ -e src/$PROJECT/manage.py ] ; then
        (cd bin ; ln -s ../src/$PROJECT/manage.py manage.py)
...
if [ ! -L static/media ] ; then
    cd static
    ln -s ../src/$PROJECT/media media
    cd ..
fi

./bin/pip install -I -r src/$PROJECT/requirements.txt
